...
const double cte_weight     = 25;     // centered on track
const double epsi_weight    = 1000;   // in road direction
const double v_weight       = 1;      // maintain speed constant or near constant
const double delta_weight   = 10;     // steering rate
const double a_weight       = 1;      // acceleration
const double da_weight      = 1;      // smooth out the acceleration
const double ddelta_weight  = 10000;  // smooth out the steering rate
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for(int t_step = 0; t_step < N; ++t_step) {
      ...
      fg[0] += v_weight * CppAD::pow(vars[v_start + t_step] - ref_v, 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // No limits for all variables except turn angle and acceleration
  for (int i = 0; i < delta_start; i++) {
    // initialize to minimum or maximum value, accordingly
    vars_lowerbound[i] = -1.0e19;
    vars_upperbound[i] = 1.0e19;
  }

  // turn angle limits (25 degrees, in radians)
  ...
  // set the upper and lower limits of delta (values in radians).
  // 25 deg in radians - 0.436332
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.436332;
    vars_upperbound[i] = 0.436332;
  }
