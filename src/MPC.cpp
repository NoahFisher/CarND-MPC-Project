...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // these are the steering and acceleration to do in the next timestep
  this->steering_angle = solution.x[delta_start];
  this->throttle = solution.x[a_start];

  cout << "steering_angle: " << steering_angle << " throttle: " << throttle << endl;

  for(int i = 0; i < N-1; i++)
  {
    // even values for x, odd values for y (this is the "green line" in the sim)
